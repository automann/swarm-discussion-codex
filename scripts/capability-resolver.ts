...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ...
  : existsSync(projectCli)
    ? [projectCli, "run", "capability-resolver"]
  : ["local-repo-harness", "run", "capability-resolver"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper capability-resolver: ${result.error.message}`);
  process.exit(1);
}
