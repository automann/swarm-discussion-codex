...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "contract-run"]
  : existsSync(projectCli)
    ? [projectCli, "run", "contract-run"]
  : ["local-repo-harness", "run", "contract-run"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper contract-run: ${result.error.message}`);
  process.exit(1);
}
