...
if [[ -x "$PROJECT_CLI" ]]; then
  exec "$PROJECT_CLI" run check-architecture-sync "$@"
fi

if command -v local-repo-harness >/dev/null 2>&1; then
  exec local-repo-harness run check-architecture-sync "$@"
fi

echo "Missing local-repo-harness CLI for helper check-architecture-sync" >&2
exit 1
