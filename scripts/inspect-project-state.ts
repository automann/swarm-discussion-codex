...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "inspect-project-state"]
  : existsSync(projectCli)
    ? [projectCli, "run", "inspect-project-state"]
  : ["local-repo-harness", "run", "inspect-project-state"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper inspect-project-state: ${result.error.message}`);
  process.exit(1);
}
