...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "capability-config"]
  : existsSync(projectCli)
    ? [projectCli, "run", "capability-config"]
  : ["local-repo-harness", "run", "capability-config"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper capability-config: ${result.error.message}`);
  process.exit(1);
}
