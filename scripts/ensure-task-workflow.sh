...
if [[ -n "$SOURCE_ROOT" && -f "$SOURCE_ROOT/src/cli/index.ts" ]]; then
  if command -v bun >/dev/null 2>&1; then
    exec bun "$SOURCE_ROOT/src/cli/index.ts" run ensure-task-workflow "$@"
  fi
fi

if [[ -x "$PROJECT_CLI" ]]; then
  exec "$PROJECT_CLI" run ensure-task-workflow "$@"
fi

if command -v local-repo-harness >/dev/null 2>&1; then
  exec local-repo-harness run ensure-task-workflow "$@"
fi

echo "Missing local-repo-harness CLI for helper ensure-task-workflow" >&2
exit 1
