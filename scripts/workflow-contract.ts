...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "workflow-contract"]
  : existsSync(projectCli)
    ? [projectCli, "run", "workflow-contract"]
  : ["local-repo-harness", "run", "workflow-contract"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper workflow-contract: ${result.error.message}`);
  process.exit(1);
}
