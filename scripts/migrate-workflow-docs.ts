...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "migrate-workflow-docs"]
  : existsSync(projectCli)
    ? [projectCli, "run", "migrate-workflow-docs"]
  : ["local-repo-harness", "run", "migrate-workflow-docs"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper migrate-workflow-docs: ${result.error.message}`);
  process.exit(1);
}
