...
if [[ -n "$SOURCE_ROOT" && -f "$SOURCE_ROOT/src/cli/index.ts" ]]; then
  if command -v bun >/dev/null 2>&1; then
    exec bun "$SOURCE_ROOT/src/cli/index.ts" run verify-sprint "$@"
  fi
fi

if [[ -x "$PROJECT_CLI" ]]; then
  exec "$PROJECT_CLI" run verify-sprint "$@"
fi

if command -v local-repo-harness >/dev/null 2>&1; then
  exec local-repo-harness run verify-sprint "$@"
fi

echo "Missing local-repo-harness CLI for helper verify-sprint" >&2
exit 1
