...
const command = sourceRoot && existsSync(join(sourceRoot, "src", "cli", "index.ts"))
  ? ["bun", join(sourceRoot, "src", "cli", "index.ts"), "run", "check-skill-version"]
  : existsSync(projectCli)
    ? [projectCli, "run", "check-skill-version"]
  : ["local-repo-harness", "run", "check-skill-version"];
...
if (result.error) {
  console.error(`Missing local-repo-harness CLI for helper check-skill-version: ${result.error.message}`);
  process.exit(1);
}
