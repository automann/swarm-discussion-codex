...
function readPolicy(repo: string): {
  hook_source?: unknown;
  helper_source?: unknown;
  harness?: { helper_source?: unknown };
  host_adapters?: { scope?: unknown };
} {
  try {
...
function classifyHostAdapter(repo: string, adapterPath: string, preserveManagedHooks: boolean): RuntimeReclaimFile | null {
  ...
  try {
    ...
    if (preserveManagedHooks) {
      return {
        path: adapterPath,
        category: 'repo-local-host-adapter',
        classification: 'managed-entry',
        action: 'preserve',
        reason: 'repo-local managed hooks are active for project-scoped adapters',
      };
    }
    return {
      path: adapterPath,
...
function buildPlan(opts: RuntimeReclaimOptions, repo: string): RuntimeReclaimResult {
  ...
  const preserveRepoLocalManagedAdapters = hookSourceRepo || policy.host_adapters?.scope === 'project';
  const files: RuntimeReclaimFile[] = [];
  ...
  for (const adapter of ['.claude/settings.json', '.claude/settings.local.json', '.codex/hooks.json']) {
    const entry = classifyHostAdapter(repo, adapter, preserveRepoLocalManagedAdapters);
    if (entry) files.push(entry);
  }
