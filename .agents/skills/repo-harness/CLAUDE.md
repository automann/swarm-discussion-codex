...
- `plans/prds/` for upper-layer PRDs; `plans/sprints/` for ordered sprint backlogs operated through `scripts/sprint-backlog.sh` (default package-mode helpers dispatch through root `scripts/` wrappers; repo-pinned helper runtime may live under `.ai/harness/scripts/`; task contracts stay the execution slices)
